...
function shallowCopy(obj) {
  const target = {};
  const resultObj = Object.assign(target, obj);
  return resultObj;
}
...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    tempArr = Object.entries(objects[i]);
    for (let j = 0; j < tempArr.length; j += 1) {
      const [key, value] = tempArr[j];
      if (resultObj[key]) {
        resultObj[key] += value;
      } else {
        resultObj[key] = value;
...
function sortCitiesArray(arr) {
  const result = arr
    ...
    .sort((c, d) => {
      if (c.country === d.country && c.city > d.city) {
        return 1;
      }
      if (c.country === d.country && c.city < d.city) {
        return -1;
